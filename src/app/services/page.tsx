      ...
      <main className="flex-1 pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesData.map((service, index) => (
            <div
              key={service.id}
              className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-red-500/50 shadow-xl overflow-hidden flex flex-col justify-between group transition-all duration-300"
            >
              ...
              <div className="p-6 sm:p-8 space-y-4 flex-1 flex flex-col justify-between font-inter">
                ...
                <div className="pt-4 flex items-center justify-between gap-3 border-t border-slate-100 dark:border-slate-800 font-manrope">
                  <button
                    onClick={() => openBookModal(service.title)}
                    className="px-4 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-500 font-bold text-xs transition-colors"
                  >
                    Request Quote
                  </button>
                  ...
                  <Link
                    ...
                    className="text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-red-500 flex items-center gap-1 group/link"
                  >
                    Learn More
